...
export class Profit {
  calculateProfit = (...args: [number | string, number | string]): number => {
    ...
    const [porcent, priceTotal] = args.map((value) =>
  ...
  calculateProductWithoutProfit = (
    ...args: [number | string, number | string]
  ): number => {
    const [amount, price] = args.map((value) =>
      typeof value === "string" ? parseFloat(value) : value
    );
  ...
  calculatePriceToSell = (
    ...args: [number | string, number | string, number | string]
  ): number => {
    const [profit, price, amount] = args.map((value) =>
      typeof value === "string" ? parseFloat(value) : value
    );
  ...
  converToFloat = (value: number | string): number =>
    typeof value === "string" ? parseFloat(value) : value;
  changeMoney = (valueInput: number, valueTotal: number) =>
    valueInput - valueTotal;
}

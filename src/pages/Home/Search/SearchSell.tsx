...
const SearchSell = () => {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<IDataInput>({ defaultValues: { search: "" } });
  const [classType, setClassFocus] = React.useState<IClassFocus>({
    classP: "",
  });
      ...
      <Styles.ContainerForm>
        <form onSubmit={(e: React.FormEvent) => e.preventDefault()}>
          <label className={`${classType.classP}`}>
            ...
            <span
              className={
                errors.search?.type === "required" ? "isRequiredAsteristic" : ""
              }
            >
              *
            </span>
            {errors.search?.type === "required" && (
              <p className="isRequiredLable">Campo Pesquisa é obrigatório!</p>
            )}
          </label>
